...
Deletes multiple people across a specified range from the address book. Irreversible. +
Format: `deletem START_INDEX END_INDEX`

****
Deletes all people from `START_INDEX` to `END_INDEX`.
The index refers to the index number shown in the most recent listing.
****
...
* `find Betsy` +
`deletem 1 3` +
Deletes the 1st, 2nd, 3rd person in the results of the `find` command.
